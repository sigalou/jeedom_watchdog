...
{
    $device = watchdog::byLogicalId($item['deviceSerialNumber'], 'watchdog');
	if (is_object($device)) {
...
//	$('#md_modal').load('index.php?v=d&plugin=watchdog&modal=history&id=watchdog&size='+selectedSize).dialog('open');
	$('#md_modal').load('index.php?v=d&plugin=watchdog&modal=history&id=watchdog').dialog('open');
});
</script>
