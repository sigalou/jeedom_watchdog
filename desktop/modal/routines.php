...
log::add('watchdog', 'debug', '=============================================================Routines');
...
<table class="table table-condensed tablesorter" id="table_healthNetwork">
	...
	foreach($json as $item)
	{
		...
		if ($item['status'] == 'ENABLED'){
			$couleur="success";
			$present = '<span class="label label-'.$couleur.'" style="font-size : 1em;" title="{{Actif}}"><i class="fa fa-check-circle"></i></span>';
		} else {
			$couleur="default";
			$present = '<span class="label label-'.$couleur.'" style="font-size : 1em;" title="{{Inactif}}"><i class="fa fa-times-circle"></i></span>';
		}		
...
$('.RunRoutine').off('click').on('click',function(){
	...
	jeedom.plugin.node.action2({
		...
		error: function (error) {
			...
			$('#md_modal').load('index.php?v=d&plugin=watchdog&modal=routines&id=watchdog').dialog('open');
		},
