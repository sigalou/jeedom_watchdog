	...
	<div class="col-lg-2 col-md-3 col-sm-4 col-xs-4">
		<div class="bs-sidebar">
			<ul id="ul_eqLogic" class="nav nav-list bs-sidenav">
				<a class="btn btn-default watchdogMasterAction" style="width : 100%;margin-top : 5px;margin-bottom: 5px;" data-action="add"><i class="fa fa-plus-circle"></i> {{Ajouter un Jeedom cible}}</a>
...
?>
	...
	<div class="col-lg-10 col-md-9 col-sm-8 col-xs-8 watchdogMaster" style="border-left: solid 1px #EEE; padding-left: 25px;display:none;">
		<a class="btn btn-success watchdogMasterAction pull-right" data-action="save"><i class="fa fa-check-circle"></i> {{Sauvegarder}}</a>
		<a class="btn btn-danger watchdogMasterAction pull-right" data-action="remove"><i class="fa fa-minus-circle"></i> {{Supprimer}}</a>
		...
		<ul class="nav nav-tabs" role="tablist">
			...
			<li role="presentation"><a href="#watchdogMasterAffecttab" aria-controls="profile" role="tab" data-toggle="tab"><i class="fa fa-list-alt"></i> {{Affectation}}</a></li>
		</ul>
		...
		<div class="tab-content" style="height:calc(100% - 50px);overflow:auto;overflow-x: hidden;">
			...
			<div role="tabpanel" class="tab-pane" id="watchdogMasterAffecttab">
				<a class="btn btn-success btn-sm pull-right" id="bt_watchdogMasterAddEqLogic"><i class="fa fa-plus-circle"></i> {{Ajouter un équipement}}</a><br>
				<form class="form-horizontal">
					<fieldset>
...
<script>
	...
	function addwatchdogMasterEqLogic(_eqLogic){
		...
		div += '<a class="btn btn-default bt_removewatchdogMasterEqLogic btn-sm"><i class="fa fa-minus-circle"></i></a>';
		div += '</span>';
	...
	function displaywatchdogMaster(_id){
		...
		$.ajax({
			...
			success: function (data) {
				if (data.state != 'ok') {
					$('#div_watchdogMasterAlert').showAlert({message: data.result, level: 'danger'});
					return;
				}
				$('.watchdogMaster').show();
	...
	$('.watchdogMasterAction[data-action=save]').on('click',function(){
		...
		$.ajax({
			...
			success: function (data) {
				...
				$('#div_watchdogMasterAlert').showAlert({message: '{{Sauvegarde réussie}}', level: 'success'});
